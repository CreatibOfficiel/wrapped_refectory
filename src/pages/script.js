...
function displayNoOrdersMessage() {
    ...
    slidesContainer.innerHTML = '<p class="text-xl text-gray-500">Aucune commande trouvée pour cette année.</p>';
}
...
function calculateTopDishes(orders) {
    ...
    orders.forEach(order => {
        if (order.products.length > 0) {
            ...
            firstProductCount[firstProduct] = (firstProductCount[firstProduct] || 0) + 1;
        }
    });
...
function calculateFavoriteDessert(orders) {
    ...
    const [dessertTitle, dessertCountValue] = Object.entries(dessertCount)
        .sort((a, b) => b[1] - a[1])[0];
...
function calculateAverageDeliveryHour(orders, totalOrders) {
    ...
    const averageDeliveryTime = totalOrders > 0 ? totalDeliveryMinutes / totalOrders : 0;
...
function generateSlides(yearData) {
    return [
        ...
        {
            text: `En moyenne, vous avez dépensé <span class="text-4xl font-bold text-green-600">${yearData.averageSpent.toFixed(2)}€</span> par commande, avec une livraison à <span class="text-4xl font-bold text-green-600">${yearData.averageDeliveryHour}</span>.<br>Toujours à l'heure ! (pas comme la SNCF 🤫)`
        },
        {
...
function initializeSlideshow(slides) {
    ...
    function renderPodiumSlide(slide) {
        ...
        podiumElement.innerHTML = `
            ...
            <p class="mt-4 text-lg">
                Votre plat fétiche ? Le <span class="text-4xl font-bold text-green-600">${slide.topDishes[0].title}</span> !
                Commandé <span class="text-4xl font-bold text-green-600">${slide.topDishes[0].count}</span> fois.<br>
    ...
    function renderStandardSlide(slide) {
        ...
        slideElement.classList.add(
            ...
            'duration-500', 'opacity-0', 'animate-fadeIn'
        );
